...
header_names = ['filename', 'OK', 'AirRoomShake', 'Dead', 'Empty', 'NoAirRoom', 'Split', 'Weak', 'Flower']
...
def init_incremental_dataset_spliting(old_dir, output_dir):
    ...
    incremental_df = pd.concat([old_train_df, old_test_df, train_df, test_df]).drop_duplicates(keep=False)
    ...
    for class_name in header_names[1:]:
        ...
        incremental_train_df.append(class_samples.sample(n=len(class_samples) // 4 * 3) )
    incremental_train_df = pd.concat(incremental_train_df)
    # the remained samples are for incremental testing.
    incremental_test_df = pd.concat([incremental_train_df, incremental_df]).drop_duplicates(keep=False)
    print(f'Old samples {len(old_df)}, for training {len(old_train_df)}, for testing {len(old_test_df)}')
    print(f'Incremental samples {len(incremental_df)}, for training {len(incremental_train_df)}, for testing {len(incremental_test_df)}')
    train_df = pd.concat([old_train_df, incremental_train_df])
    test_df = pd.concat([old_test_df, incremental_test_df])
    train_df.to_csv(os.path.join(output_dir, 'train.csv'), index=False)
    test_df.to_csv(os.path.join(output_dir, 'test.csv'), index=False)
    total_df = pd.concat([train_df, test_df])
    print(f'New samples {len(total_df)}, for traning {len(train_df)}, for testing {len(test_df)}')
    for class_name in header_names[1:]:
        class_samples = total_df.loc[total_df['filename'].str.startswith(class_name)] 
...
def add_intermediate_csv(df : pd.DataFrame, output_dir, output, intermediate = 'egg_roi'):
...
def generate_from_recent_csv(input_dir, output_dir):
    ...
    add_intermediate_csv(train_df, output_dir, os.path.join(output_dir, 'train.csv'))
    add_intermediate_csv(test_df, output_dir, os.path.join(output_dir, 'test.csv'))
